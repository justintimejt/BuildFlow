...
import { Template, getLocalTemplates, getTemplateById } from '../utils/templates';
...
export const useTemplates = () => {
  ...
  const createProjectFromTemplate = useCallback((templateId: string, projectName?: string): Project | null => {
    ...
    const newNodes = template.project.nodes.map(node => ({
      ...node,
      id: uuidv4()
    }));
    ...
    template.project.nodes.forEach((oldNode, index) => {
      nodeIdMap.set(oldNode.id, newNodes[index].id);
    });

    const newEdges = template.project.edges.map(edge => ({
      ...edge,
      id: uuidv4(),
